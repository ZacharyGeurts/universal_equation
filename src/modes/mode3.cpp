...
void renderMode3(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer,
    ...
    // Project 9D ball positions into 3D space with spiral motion
    std::vector<float> vertexData;
    vertexData.reserve(balls.size() * 6); // Position (x, y, z) + Normal (x, y, z) per ball
    for (size_t i = 0; i < balls.size(); ++i) {
        const auto& ball = balls[i];
        // Spiral projection: modulate x, y, z with wavePhase and index-based offset
        float angle = wavePhase * 2.0f + static_cast<float>(i) * 0.01f;
        float radius = 1.0f + 0.2f * sin(wavePhase * 3.0f);
        float x = ball.x * radius * cos(angle);
        float y = ball.y * radius * sin(angle);
        float z = ball.z * (1.0f + 0.3f * sin(wavePhase * 2.5f));
    ...
    // Begin render pass
    VkClearValue clearColor = {{{0.0f, 0.05f, 0.1f, 1.0f}}}; // Dark navy background for spiral effect
    ...
    if (VkResult result = vkBeginCommandBuffer(commandBuffer, &beginInfo); result != VK_SUCCESS) {
        amouranth->getLogger().log(Logging::LogLevel::Error, "RenderMode3", "Failed to begin command buffer: result={}",
    ...
    float musicZoom = zoomLevel * (1.0f + 0.2f * sin(wavePhase * 2.0f + randomShift));
    glm::mat4 proj = glm::perspective(glm::radians(50.0f * musicZoom), aspectRatio, 0.1f, 1000.0f);
    glm::mat4 view = glm::lookAt(
        glm::vec3(
            sin(wavePhase * 0.8f + randomShift) * 5.0f,
            cos(wavePhase * 0.8f + randomShift) * 3.0f,
            -6.0f
        ),
        glm::vec3(0.0f, 0.0f, 0.0f),
        glm::vec3(0.0f, 1.0f, 0.0f)
    );
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.5f + randomShift, glm::vec3(1.0f, 1.0f, 0.0f));

    pushConstants.mvp = proj * view * model;
    float beatIntensity = cache.empty() ? 1.0f : cache[0].value;
    pushConstants.beatIntensity = beatIntensity * (1.0f + 0.5f * abs(sin(wavePhase * 2.5f + randomShift)));
    pushConstants.amplitude = 1.0f + cos(wavePhase * 2.5f + randomShift) * 0.6f;
    pushConstants.time = wavePhase;
    pushConstants.baseColor = glm::vec3(
        0.6f + sin(wavePhase * 1.0f + randomShift) * 0.3f,
        0.4f + cos(wavePhase * 1.2f + randomShift) * 0.3f,
    ...
    // Additional elements (offset spiral layer for depth)
    model = glm::rotate(glm::mat4(1.0f), wavePhase * -0.5f + randomShift, glm::vec3(1.0f, 1.0f, 0.0f));
    model = glm::translate(model, glm::vec3(0.0f, 0.0f, sin(wavePhase * 0.4f) * 1.0f)); // Depth offset
    pushConstants.mvp = proj * view * model;
    pushConstants.baseColor = glm::vec3(
        0.4f + cos(wavePhase * 1.0f) * 0.3f,
        0.6f + sin(wavePhase * 1.2f) * 0.3f,
        0.5f + cos(wavePhase * 1.4f) * 0.3f
    );
