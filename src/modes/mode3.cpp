...
void renderMode3(const AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer,
    ...
    // Project 9D ball positions onto 3D space (x, y, z with pulsating spherical effect)
    std::vector<float> vertexData;
    vertexData.reserve(balls.size() * 6); // Position (x, y, z) + Normal (x, y, z) per ball
    for (const auto& ball : balls) {
        // Project 9D position to 3D (pulsating spherical effect)
        float scale = 1.0f + sin(wavePhase * 2.5f) * 0.2f; // Pulsating scale
        float x = ball.x * scale;
        float y = ball.y * scale;
        float z = ball.z * scale;
        vertexData.push_back(x);
        vertexData.push_back(y);
        vertexData.push_back(z);
        vertexData.push_back(x); // Normal approximates position for spherical look
        vertexData.push_back(y);
        vertexData.push_back(z);
    }
    ...
    // Setup push constants (similar to mode1/mode2 but with 3D parameters)
    ...
    glm::mat4 view = glm::lookAt(
        glm::vec3(
            sin(wavePhase * 0.7f + randomShift) * 2.0f, // Wider orbit
            cos(wavePhase * 0.7f + randomShift) * 2.0f,
            -10.0f // Farther back for 3D
        ),
        glm::vec3(0.0f, 0.0f, 0.0f),
        glm::vec3(0.0f, 1.0f, 0.0f)
    );
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.4f + randomShift, glm::vec3(1.0f, 1.0f, 1.0f)); // 3D rotation

    pushConstants.mvp = proj * view * model;
    float beatIntensity = cache.empty() ? 1.0f : cache[0].value;
    pushConstants.beatIntensity = beatIntensity * (1.0f + 0.3f * abs(sin(wavePhase * 2.5f + randomShift))); // Gentle pulsing
    ...
    pushConstants.baseColor = glm::vec3(
        0.5f + sin(wavePhase * 1.0f + randomShift) * 0.5f, // Softer color phasing
        0.5f + cos(wavePhase * 1.0f + randomShift) * 0.5f,
        0.5f + sin(wavePhase * 1.3f + randomShift) * 0.3f
    );
    ...
    // Additional elements (offset layer for depth effect)
    model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.6f + randomShift, glm::vec3(1.0f, 1.0f, 1.0f));
    model = glm::translate(model, glm::vec3(0.0f, 0.0f, sin(wavePhase * 0.3f) * 0.5f)); // Depth pulse
    pushConstants.mvp = proj * view * model;
    pushConstants.baseColor = glm::vec3(
        0.5f + cos(wavePhase * 1.0f) * 0.5f,
        0.5f + sin(wavePhase * 1.0f) * 0.5f,
        0.5f + cos(wavePhase * 1.3f) * 0.3f
    );
