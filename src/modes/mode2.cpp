...
void renderMode2(const AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer,
    ...
    // Project 9D ball positions onto 2D plane (x-y plane, modulated by wavePhase)
    std::vector<float> vertexData;
    vertexData.reserve(balls.size() * 6); // Position (x, y, z) + Normal (x, y, z) per ball
    for (const auto& ball : balls) {
        // Project 9D position to 2D (x-y plane with swirling effect)
        float x = ball.x * (1.0f + cos(wavePhase * 3.0f) * 0.3f); // Swirling x-motion
        float y = ball.y * (1.0f + sin(wavePhase * 3.0f) * 0.3f); // Swirling y-motion
    ...
    // Setup push constants (similar to mode1 but with different visual parameters)
    ...
    float musicZoom = zoomLevel * (1.0f + 0.3f * cos(wavePhase * 3.0f + randomShift)); // Smoother zoom for mode 2
    glm::mat4 proj = glm::ortho(-aspectRatio * musicZoom, aspectRatio * musicZoom, -1.0f * musicZoom, 1.0f * musicZoom, 0.1f, 1000.0f);
    glm::mat4 view = glm::lookAt(
        glm::vec3(
            cos(wavePhase * 0.8f + randomShift) * 0.6f, // Slower swirling motion
            sin(wavePhase * 0.8f + randomShift) * 0.6f,
            -5.0f
        ),
    ...
    pushConstants.beatIntensity = beatIntensity * (1.0f + 0.4f * abs(cos(wavePhase * 3.0f + randomShift))); // Softer pulsing
    pushConstants.amplitude = 1.0f + cos(wavePhase * 3.0f + randomShift) * 0.7f; // Reduced amplitude
    pushConstants.time = wavePhase;
    pushConstants.baseColor = glm::vec3(
        0.5f + cos(wavePhase * 1.1f + randomShift) * 0.5f, // Different color phasing
        0.5f + sin(wavePhase * 1.1f + randomShift) * 0.5f,
        0.5f + cos(wavePhase * 1.4f + randomShift) * 0.3f
    );
    ...
    model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.7f + randomShift, glm::vec3(0.0f, 0.0f, 1.0f)); // Rotate for swirl
    model = glm::translate(model, glm::vec3(cos(wavePhase * 0.4f) * 0.3f, sin(wavePhase * 0.4f) * 0.3f, 0.0f)); // Orbital motion
    pushConstants.mvp = proj * view * model;
    pushConstants.baseColor = glm::vec3(
        0.5f + sin(wavePhase * 1.1f) * 0.5f,
        0.5f + cos(wavePhase * 1.1f) * 0.5f,
        0.5f + sin(wavePhase * 1.4f) * 0.3f
    );
    pushConstants.amplitude *= 0.8f; // Reduced for second layer
    vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PushConstants), &pushConstants);
    vkCmdDrawIndexed(commandBuffer, indexCount, 1, 0, 0, 0);
