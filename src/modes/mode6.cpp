...
void renderMode6(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer,
    ...
    for (size_t i = 0; i < balls.size(); ++i) {
        ...
        // Vortex projection: create a funnel-like effect with dynamic scaling
        float angle = wavePhase * 3.0f + static_cast<float>(i) * 0.02f;
        float radius = (1.0f + 0.3f * cos(wavePhase * 2.5f)) * (1.0f - ball.z * 0.1f);
        float x = ball.x * radius * cos(angle);
        float y = ball.y * radius * sin(angle);
        float z = ball.z * (1.0f + 0.4f * sin(wavePhase * 2.0f));
    ...
    // Begin render pass
    VkClearValue clearColor = {{{0.02f, 0.02f, 0.1f, 1.0f}}}; // Deep blue background for vortex effect
    ...
    float musicZoom = zoomLevel * (1.0f + 0.25f * sin(wavePhase * 3.0f + randomShift));
    glm::mat4 proj = glm::perspective(glm::radians(55.0f * musicZoom), aspectRatio, 0.1f, 1000.0f);
    ...
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.6f + randomShift, glm::vec3(0.5f, 0.5f, 1.0f));

    pushConstants.mvp = proj * view * model;
    float beatIntensity = cache.empty() ? 1.0f : cache[0].value;
    pushConstants.beatIntensity = beatIntensity * (1.0f + 0.5f * abs(cos(wavePhase * 3.0f + randomShift)));
    ...
    pushConstants.baseColor = glm::vec3(
        0.5f + sin(wavePhase * 1.1f + randomShift) * 0.4f,
    ...
    // Additional elements (offset vortex layer for depth)
    model = glm::rotate(glm::mat4(1.0f), wavePhase * -0.6f + randomShift, glm::vec3(0.5f, 0.5f, 1.0f));
    model = glm::translate(model, glm::vec3(0.0f, 0.0f, cos(wavePhase * 0.5f) * 1.5f)); // Depth offset
    pushConstants.mvp = proj * view * model;
    pushConstants.baseColor = glm::vec3(
        0.5f + cos(wavePhase * 1.1f) * 0.4f,
        0.5f + sin(wavePhase * 1.3f) * 0.4f,
        0.5f + cos(wavePhase * 1.5f) * 0.4f
    );
