...
void renderMode6(const AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer,
    ...
    // Project 9D ball positions into 3D space with vortex effect
    std::vector<float> vertexData;
    vertexData.reserve(balls.size() * 6); // Position (x, y, z) + Normal (x, y, z) per ball
    for (const auto& ball : balls) {
        // Apply vortex effect (spiral around z-axis)
        float angle = wavePhase * 2.0f + ball.z; // Spiral based on z
        float radius = 1.0f + cos(wavePhase * 2.0f) * 0.2f; // Pulsating radius
        ...
        vertexData.push_back(x); // Normal approximates position for vortex look
        vertexData.push_back(y);
        vertexData.push_back(z);
    }
    ...
    if (VkResult result = vkBeginCommandBuffer(commandBuffer, &beginInfo); result != VK_SUCCESS) {
        amouranth->getLogger().log(Logging::LogLevel::Error, "RenderMode6", "Failed to begin command buffer: result={}",
    ...
    // Setup push constants (vortex effect)
    ...
    glm::mat4 view = glm::lookAt(
        glm::vec3(
            cos(wavePhase * 0.6f + randomShift) * 3.0f, // Wider vortex orbit
            sin(wavePhase * 0.6f + randomShift) * 3.0f,
            -15.0f // Farther back for 3D
        ),
        glm::vec3(0.0f, 0.0f, 0.0f),
        glm::vec3(0.0f, 1.0f, 0.0f)
    );
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.5f + randomShift, glm::vec3(0.0f, 1.0f, 0.0f)); // Vortex rotation

    pushConstants.mvp = proj * view * model;
    float beatIntensity = cache.empty() ? 1.0f : cache[0].value;
    pushConstants.beatIntensity = beatIntensity * (1.0f + 0.3f * abs(cos(wavePhase * 2.0f + randomShift))); // Vortex pulsing
    pushConstants.amplitude = 1.0f + cos(wavePhase * 2.0f + randomShift) * 0.5f; // Moderate amplitude
    pushConstants.time = wavePhase;
    pushConstants.baseColor = glm::vec3(
        0.5f + cos(wavePhase * 0.7f + randomShift) * 0.5f, // Vortex color phasing
        0.5f + sin(wavePhase * 0.7f + randomShift) * 0.5f,
        0.5f + cos(wavePhase * 1.0f + randomShift) * 0.3f
    );
    ...
    // Additional elements (offset vortex layer)
    model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.7f + randomShift, glm::vec3(0.0f, 1.0f, 0.0f));
    model = glm::translate(model, glm::vec3(0.0f, 0.0f, cos(wavePhase * 0.4f) * 0.5f)); // Vortex depth
    pushConstants.mvp = proj * view * model;
    pushConstants.baseColor = glm::vec3(
        0.5f + sin(wavePhase * 0.7f) * 0.5f,
        0.5f + cos(wavePhase * 0.7f) * 0.5f,
        0.5f + sin(wavePhase * 1.0f) * 0.3f
    );
