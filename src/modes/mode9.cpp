...
void renderMode9(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, [[maybe_unused]] VkBuffer vertexBuffer, [[maybe_unused]] VkCommandBuffer commandBuffer,
    ...
    // Project 9D ball positions onto 3D chaotic attractor (Lorenz-like)
    std::vector<float> vertexData;
    vertexData.reserve(balls.size() * 6); // Position (x, y, z) + Normal (x, y, z) per ball
    for (const auto& ball : balls) {
        // Chaotic attractor mapping (simplified Lorenz system)
        float t = ball.position.x * 3.14159f + wavePhase;
        float scale = 1.0f + 0.25f * sin(wavePhase * 2.4f); // Dynamic scaling
        float x = scale * sin(t + ball.position.y * 10.0f); // Lorenz-like x
        float y = scale * cos(t + ball.position.z * 28.0f); // Lorenz-like y
        float z = scale * (sin(t + ball.position.x * 8.0f / 3.0f)); // Lorenz-like z
        vertexData.push_back(x);
        vertexData.push_back(y);
        vertexData.push_back(z);
        vertexData.push_back(sin(t)); // Normal based on trajectory
        vertexData.push_back(cos(t));
    ...
    VkClearValue clearColor = {{{0.0f, 0.05f, 0.15f, 1.0f}}}; // Dark indigo background
    renderPassInfo.clearValueCount = 1;
    renderPassInfo.pClearValues = &clearColor;
    vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);

    // Use Mia for random numbers (chaotic attractor with MiaMakesMusic flair)
    ...
    glm::vec3 cameraPos = glm::vec3(
        sin(wavePhase * 1.5f + randomShift) * 4.0f,
        cos(wavePhase * 1.5f + randomShift) * 4.0f,
        -5.0f + sin(wavePhase * 2.4f + randomShift) * 1.0f
    );
    glm::mat4 view = glm::lookAt(cameraPos, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.9f + randomShift, glm::vec3(0.5f, 0.5f, 1.0f));

    pushConstants.mvp = proj * view * model;
    float nurbEnergy = cache.empty() ? 1.0f : static_cast<float>(cache[0].nurbEnergy);
    pushConstants.beatIntensity = nurbEnergy * (1.0f + 0.5f * abs(cos(wavePhase * 3.6f + randomShift)));
    pushConstants.amplitude = 1.0f + sin(wavePhase * 2.5f + randomShift) * 0.7f;
    pushConstants.time = wavePhase;
    pushConstants.baseColor = glm::vec3(
        0.4f + sin(wavePhase * 1.6f + randomShift) * 0.3f,
        0.6f + cos(wavePhase * 1.4f + randomShift) * 0.3f,
        0.8f + sin(wavePhase * 1.8f + randomShift) * 0.2f
    );
    ...
    // Draw indexed (for point-based chaotic attractor geometry)
    uint32_t indexCount = static_cast<uint32_t>(indices.size());
    vkCmdDrawIndexed(commandBuffer, indexCount, 1, 0, 0, 0);

    // Additional elements (scaled chaotic attractor layer)
    model = glm::rotate(glm::mat4(1.0f), -wavePhase * 0.9f, glm::vec3(0.5f, 0.5f, 1.0f));
    model = glm::scale(model, glm::vec3(0.8f, 0.8f, 0.8f));
    pushConstants.mvp = proj * view * model;
    pushConstants.baseColor = glm::vec3(
        0.6f + cos(wavePhase * 1.6f) * 0.2f,
        0.4f + sin(wavePhase * 1.4f) * 0.3f,
        0.8f + cos(wavePhase * 1.8f) * 0.2f
    );
