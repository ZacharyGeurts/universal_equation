...
void renderMode5(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, [[maybe_unused]] VkBuffer vertexBuffer, [[maybe_unused]] VkCommandBuffer commandBuffer,
    ...
    // Project 9D ball positions onto 3D spherical shell
    std::vector<float> vertexData;
    vertexData.reserve(balls.size() * 6); // Position (x, y, z) + Normal (x, y, z) per ball
    for (const auto& ball : balls) {
        // Spherical coordinates
        float theta = ball.position.x * 2.0f * 3.14159f + wavePhase; // Azimuthal angle
        float phi = ball.position.y * 3.14159f; // Polar angle
        float radius = 1.0f + 0.25f * sin(wavePhase * 2.8f); // Dynamic radius
        ...
        vertexData.push_back(x / radius); // Normal is position normalized
    ...
    VkClearValue clearColor = {{{0.0f, 0.1f, 0.1f, 1.0f}}}; // Teal background
    renderPassInfo.clearValueCount = 1;
    renderPassInfo.pClearValues = &clearColor;
    vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);

    // Use Mia for random numbers (spherical fractal with MiaMakesMusic flair)
    ...
    glm::vec3 cameraPos = glm::vec3(
        sin(wavePhase * 1.1f + randomShift) * 3.5f,
        cos(wavePhase * 1.1f + randomShift) * 3.5f,
        sin(wavePhase * 1.8f + randomShift) * 1.2f
    );
    glm::mat4 view = glm::lookAt(cameraPos, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.85f + randomShift, glm::vec3(0.0f, 1.0f, 0.5f));

    pushConstants.mvp = proj * view * model;
    float nurbEnergy = cache.empty() ? 1.0f : static_cast<float>(cache[0].nurbEnergy);
    pushConstants.beatIntensity = nurbEnergy * (1.0f + 0.45f * abs(sin(wavePhase * 3.2f + randomShift)));
    pushConstants.amplitude = 1.0f + sin(wavePhase * 2.7f + randomShift) * 0.7f;
    pushConstants.time = wavePhase;
    pushConstants.baseColor = glm::vec3(
        0.5f + cos(wavePhase * 1.4f + randomShift) * 0.4f,
        0.7f + sin(wavePhase * 1.2f + randomShift) * 0.3f,
        0.4f + cos(wavePhase * 1.6f + randomShift) * 0.3f
    );
    ...
    // Draw indexed (for point-based spherical geometry)
    uint32_t indexCount = static_cast<uint32_t>(indices.size());
    vkCmdDrawIndexed(commandBuffer, indexCount, 1, 0, 0, 0);

    // Additional elements (inner spherical layer)
    model = glm::rotate(glm::mat4(1.0f), -wavePhase * 0.85f, glm::vec3(0.0f, 1.0f, 0.5f));
    model = glm::scale(model, glm::vec3(0.75f, 0.75f, 0.75f));
    pushConstants.mvp = proj * view * model;
    pushConstants.baseColor = glm::vec3(
        0.7f + sin(wavePhase * 1.4f) * 0.3f,
        0.4f + cos(wavePhase * 1.2f) * 0.3f,
        0.5f + sin(wavePhase * 1.6f) * 0.2f
    );
