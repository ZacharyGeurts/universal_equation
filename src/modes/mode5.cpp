...
void renderMode5(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, [[maybe_unused]] VkBuffer vertexBuffer, [[maybe_unused]] VkCommandBuffer commandBuffer,
    ...
    if (balls.empty()) {
        amouranth->getLogger().log(Logging::LogLevel::Error, "No ball data for renderMode5",
    ...
    // Project 9D ball positions onto 3D spherical swarm
    ...
    for (const auto& ball : balls) {
        ...
        float theta = ball.position.x * 2.0f * 3.14159f + randomShift * 0.15f;
        float phi = ball.position.y * 3.14159f + randomShift * 0.1f;
        float radius = 1.5f + sin(wavePhase * 3.5f + randomShift) * 0.3f; // Pulsing sphere
        ...
        vertexData.push_back(x / radius); // Normal x
        vertexData.push_back(y / radius); // Normal y
        vertexData.push_back(z / radius); // Normal z
    ...
    glm::vec3 cameraPos = glm::vec3(
        4.0f * sin(wavePhase * 0.5f + randomShift),
        4.0f * cos(wavePhase * 0.5f + randomShift),
        3.0f + sin(wavePhase * 0.7f + randomShift) * 0.5f
    );
    ...
    // Draw indexed (point-based sphere)
    uint32_t indexCount = static_cast<uint32_t>(indices.size());
    vkCmdDrawIndexed(commandBuffer, indexCount, 1, 0, 0, 0);

    // Kaleidoscopic effect (mirrored sphere)
    ...
    if (VkResult result = vkEndCommandBuffer(commandBuffer); result != VK_SUCCESS) {
        amouranth->getLogger().log(Logging::LogLevel::Error, "Failed to record command buffer for renderMode5: result={}",
