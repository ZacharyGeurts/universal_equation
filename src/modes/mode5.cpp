...
void renderMode5(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer,
    ...
    // Project 9D ball positions onto a 3D spherical surface with radial pulsing
    std::vector<float> vertexData;
    vertexData.reserve(balls.size() * 6); // Position (x, y, z) + Normal (x, y, z) per ball
    for (size_t i = 0; i < balls.size(); ++i) {
        const auto& ball = balls[i];
        // Spherical projection: map to a sphere with radius modulated by wavePhase
        float radius = 1.0f + 0.3f * sin(wavePhase * 3.0f + static_cast<float>(i) * 0.015f);
        float theta = ball.x * 3.14159f; // Use x for azimuthal angle
        float phi = ball.y * 1.5708f;   // Use y for polar angle
        ...
        vertexData.push_back(x / radius); // Normal points outward
    ...
    // Begin render pass
    VkClearValue clearColor = {{{0.0f, 0.0f, 0.15f, 1.0f}}}; // Dark blue background for spherical effect
    ...
    float musicZoom = zoomLevel * (1.0f + 0.2f * sin(wavePhase * 3.5f + randomShift));
    glm::mat4 proj = glm::perspective(glm::radians(60.0f * musicZoom), aspectRatio, 0.1f, 1000.0f);
    glm::mat4 view = glm::lookAt(
        glm::vec3(
            cos(wavePhase * 1.0f + randomShift) * 5.0f,
            sin(wavePhase * 1.0f + randomShift) * 5.0f,
            -8.0f
        ),
        glm::vec3(0.0f, 0.0f, 0.0f),
        glm::vec3(0.0f, 1.0f, 0.0f)
    );
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.7f + randomShift, glm::vec3(0.5f, 0.5f, 1.0f));

    pushConstants.mvp = proj * view * model;
    float beatIntensity = cache.empty() ? 1.0f : cache[0].value;
    pushConstants.beatIntensity = beatIntensity * (1.0f + 0.5f * abs(sin(wavePhase * 3.5f + randomShift)));
    pushConstants.amplitude = 1.0f + cos(wavePhase * 3.5f + randomShift) * 0.7f;
    pushConstants.time = wavePhase;
    pushConstants.baseColor = glm::vec3(
        0.5f + sin(wavePhase * 1.2f + randomShift) * 0.4f,
        0.5f + cos(wavePhase * 1.4f + randomShift) * 0.4f,
        0.5f + sin(wavePhase * 1.6f + randomShift) * 0.4f
    );
    ...
    // Additional elements (offset spherical layer for depth)
    model = glm::rotate(glm::mat4(1.0f), wavePhase * -0.7f + randomShift, glm::vec3(0.5f, 0.5f, 1.0f));
    model = glm::scale(model, glm::vec3(0.9f, 0.9f, 0.9f)); // Slightly smaller scale
    pushConstants.mvp = proj * view * model;
    pushConstants.baseColor = glm::vec3(
        0.5f + cos(wavePhase * 1.2f) * 0.4f,
        0.5f + sin(wavePhase * 1.4f) * 0.4f,
        0.5f + cos(wavePhase * 1.6f) * 0.4f
    );
