...
void renderMode8(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer,
    ...
    // Project 9D ball positions into 3D space with chaotic orbits
    std::vector<float> vertexData;
    vertexData.reserve(balls.size() * 6); // Position (x, y, z) + Normal (x, y, z) per ball
    float trajectoryScale = cache.empty() ? 1.0f : cache[0].value * 0.5f;
    for (size_t i = 0; i < balls.size(); ++i) {
        const auto& ball = balls[i];
        // Chaotic orbit: modulate positions with wavePhase and index-based offset
        float t = wavePhase * 2.0f + static_cast<float>(i) * 0.01f;
        float x = ball.x * (1.0f + trajectoryScale * sin(t * 1.5f + ball.y));
        float y = ball.y * (1.0f + trajectoryScale * cos(t * 1.7f + ball.x));
        float z = ball.z * (1.0f + trajectoryScale * sin(t * 1.3f + ball.x + ball.y));
    ...
    // Begin render pass
    VkClearValue clearColor = {{{0.0f, 0.1f, 0.05f, 1.0f}}}; // Dark teal background for chaotic effect
    ...
    float musicZoom = zoomLevel * (1.0f + 0.3f * cos(wavePhase * 2.5f + randomShift));
    glm::mat4 proj = glm::perspective(glm::radians(65.0f * musicZoom), aspectRatio, 0.1f, 1000.0f);
    glm::mat4 view = glm::lookAt(
        glm::vec3(
            sin(wavePhase * 1.1f + randomShift) * 6.0f,
            cos(wavePhase * 1.1f + randomShift) * 6.0f,
            -9.0f
        ),
        glm::vec3(0.0f, 0.0f, 0.0f),
        glm::vec3(0.0f, 1.0f, 0.0f)
    );
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.8f + randomShift, glm::vec3(0.3f, 0.7f, 1.0f));

    pushConstants.mvp = proj * view * model;
    float beatIntensity = cache.empty() ? 1.0f : cache[0].value;
    pushConstants.beatIntensity = beatIntensity * (1.0f + 0.4f * abs(cos(wavePhase * 2.5f + randomShift)));
    ...
    pushConstants.baseColor = glm::vec3(
        0.5f + cos(wavePhase * 0.9f + randomShift) * 0.4f,
        0.5f + sin(wavePhase * 1.0f + randomShift) * 0.4f,
        0.5f + cos(wavePhase * 1.1f + randomShift) * 0.4f
    );
    ...
    // Additional elements (offset orbit layer for depth)
    model = glm::rotate(glm::mat4(1.0f), wavePhase * -0.8f + randomShift, glm::vec3(0.3f, 0.7f, 1.0f));
    model = glm::translate(model, glm::vec3(sin(wavePhase * 0.5f) * 0.5f, cos(wavePhase * 0.5f) * 0.5f, 0.0f));
    pushConstants.mvp = proj * view * model;
    pushConstants.baseColor = glm::vec3(
        0.5f + sin(wavePhase * 0.9f) * 0.4f,
        0.5f + cos(wavePhase * 1.0f) * 0.4f,
        0.5f + sin(wavePhase * 1.1f) * 0.4f
    );
