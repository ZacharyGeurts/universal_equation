// mode8.cpp: Implementation of mode 8 rendering for AMOURANTH RTX Engine.
...
void renderMode8(AMOURANTH* amouranth, uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer,
    ...
    logger.log(Logging::LogLevel::Info, "Starting renderMode8 (8D) for image index {}", std::source_location::current(), imageIndex);

    // Check for valid pipeline and vertex buffer memory
    if (!pipeline || !vertexBufferMemory) {
        logger.log(Logging::LogLevel::Error, "Invalid pipeline or vertex buffer memory in renderMode8: pipeline={}, vertexBufferMemory={}",
    ...
    for (const auto& data : cache) {
        if (data.dimension == 8) { // Emphasize 8th dimension
    ...
    for (size_t i = 0; i < balls.size(); ++i) {
        ...
        float scale = ball.radius * (1.0f + static_cast<float>(energy.nurbEnergy) * 0.45f); // Stronger nurbEnergy influence for 8D
        float interactionScale = 1.0f;
        float waveAmp = 0.0f;
        if (i < interactions.size()) {
            interactionScale = static_cast<float>(interactions[i].strength) * 0.12f;
            waveAmp = static_cast<float>(interactions[i].waveAmplitude);
        }
        scale *= (1.0f + interactionScale);
        glm::vec4 color(
            0.7f + 0.3f * std::cos(waveAmp + wavePhase), // Yellow tint for 8D
            0.7f + 0.3f * std::sin(waveAmp + wavePhase),
            0.4f + 0.4f * static_cast<float>(energy.observable) / 10.0f,
            1.0f
        );
    ...
    glm::vec3 cameraPos = amouranth->isUserCamActive() ? amouranth->getUserCamPos() : glm::vec3(0.0f, 0.0f, 40.0f / zoomLevel); // Farther camera for 8D
    ...
    pushConstants.extra[0].z = static_cast<float>(energy.nurbEnergy) * 1.7f; // Stronger nurbEnergy emphasis
    ...
    logger.log(Logging::LogLevel::Info, "Completed renderMode8 (8D) for image index {}", std::source_location::current(), imageIndex);
}