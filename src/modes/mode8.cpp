...
// Renders 3 orbs in 3D for dimension 8, with physics-based bouncing and lateral motion, modulated by UniversalEquation::EnergyResult.
// Orbs move with equation-based forces and initial random velocities, bounce at aquarium edges (-5 to 5 X/Y, -2 to 2 Z).
...
const size_t kNumBalls = 300000; // Number of balls for mode 8
...
void renderMode8(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer,
    ...
    std::cout << "[DEBUG] Mode 8: Bound vertex and index buffers\n";

    // Initialize UniversalEquation for dimension 8
    static UniversalEquation equation;
    static bool initialized = false;
    if (!initialized) {
        equation.setCurrentDimension(8);
        equation.setMode(8);
        equation.setInfluence(2.5);
        ...
        if (balls.size() != kNumBalls) {
            std::cerr << "[ERROR] Mode 8: Initialized " << balls.size() << " balls, expected " << kNumBalls << "\n";
        } else {
            std::cout << "[DEBUG] Mode 8: Confirmed " << balls.size() << " balls initialized\n";
        }
    }

    // Update physics
    std::cout << "[DEBUG] Mode 8: Calling updateBalls\n";
    equation.advanceCycle();
    equation.updateBalls(0.016f);
    std::cout << "[DEBUG] Mode 8: Completed updateBalls\n";
    ...
    for (size_t i = 0; i < balls.size(); ++i) {
        ...
        if (balls[i].position.x < boundsMin.x) {
            ...
            if (equation.getDebug()) {
                std::cout << "[DEBUG] Mode 8: Ball " << i << " hit X-min boundary\n";
            }
        }
        if (balls[i].position.x > boundsMax.x) {
            ...
            balls[i].velocity.x = -balls[i].velocity.x;
            if (equation.getDebug()) {
                std::cout << "[DEBUG] Mode 8: Ball " << i << " hit X-max boundary\n";
            }
        }
        if (balls[i].position.y < boundsMin.y) {
            ...
            if (equation.getDebug()) {
                std::cout << "[DEBUG] Mode 8: Ball " << i << " hit Y-min boundary\n";
            }
        }
        if (balls[i].position.y > boundsMax.y) {
            ...
            balls[i].velocity.y = -balls[i].velocity.y;
            if (equation.getDebug()) {
                std::cout << "[DEBUG] Mode 8: Ball " << i << " hit Y-max boundary\n";
            }
        }
        if (balls[i].position.z < boundsMin.z) {
            ...
            if (equation.getDebug()) {
                std::cout << "[DEBUG] Mode 8: Ball " << i << " hit Z-min boundary\n";
            }
        }
        if (balls[i].position.z > boundsMax.z) {
            ...
            balls[i].velocity.z = -balls[i].velocity.z;
            if (equation.getDebug()) {
                std::cout << "[DEBUG] Mode 8: Ball " << i << " hit Z-max boundary\n";
            }
        }
    }
    std::cout << "[DEBUG] Mode 8: Applied boundary collisions\n";
    ...
    std::cout << "[DEBUG] Mode 8: Updated projected vertices\n";

    // Get energy data
    EnergyResult energyData = equation.compute();
    std::cout << "[DEBUG] Mode 8: Computed energy data\n";
    ...
    std::cout << "[DEBUG] Mode 8: Set up view-projection matrix\n";
    ...
    if (indexCount == 0) {
        std::cerr << "[ERROR] Mode 8: Invalid index count for sphere geometry\n";
        return;
    }
    for (size_t i = 0; i < balls.size() && i < kNumBalls; ++i) {
        ...
        std::cout << "[DEBUG] Mode 8: Pushed constants for ball " << i << "\n";
        vkCmdDrawIndexed(commandBuffer, indexCount, 1, 0, 0, 0);
        std::cout << "[DEBUG] Mode 8: Drew ball " << i << "\n";
    }

    // Export data
    if (!cache.empty()) {
        equation.exportToCSV("mode8_output.csv", cache);
        std::cout << "[DEBUG] Mode 8: Exported CSV\n";
    }
    std::cout << "[DEBUG] Exiting renderMode8\n";
}