...
void renderMode8(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, [[maybe_unused]] VkBuffer vertexBuffer, [[maybe_unused]] VkCommandBuffer commandBuffer,
    ...
    if (balls.empty()) {
        amouranth->getLogger().log(Logging::LogLevel::Error, "No ball data for renderMode8",
    ...
    // Project 9D ball positions onto 3D fractal lattice
    ...
    for (const auto& ball : balls) {
        ...
        float x = ball.position.x * 3.0f + sin(wavePhase * 3.0f + randomShift) * 0.3f;
        float y = ball.position.y * 3.0f + cos(wavePhase * 3.0f + randomShift) * 0.3f;
        float z = ball.position.z * 3.0f + sin(wavePhase * 2.5f + randomShift) * 0.3f;

        vertexData.push_back(x); // Position x
        vertexData.push_back(y); // Position y
        vertexData.push_back(z); // Position z
        vertexData.push_back(ball.position.x); // Normal x
    ...
    glm::vec3 cameraPos = glm::vec3(
        6.0f * sin(wavePhase * 0.5f + randomShift),
        6.0f * cos(wavePhase * 0.5f + randomShift),
        5.0f + sin(wavePhase * 0.8f + randomShift) * 0.7f
    );
    glm::mat4 view = glm::lookAt(cameraPos, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.7f + randomShift, glm::vec3(1.0f, 1.0f, 0.0f));
    ...
    // Draw indexed (point-based lattice)
    uint32_t indexCount = static_cast<uint32_t>(indices.size());
    vkCmdDrawIndexed(commandBuffer, indexCount, 1, 0, 0, 0);

    // Kaleidoscopic effect (mirrored lattice)
    ...
    if (VkResult result = vkEndCommandBuffer(commandBuffer); result != VK_SUCCESS) {
        amouranth->getLogger().log(Logging::LogLevel::Error, "Failed to record command buffer for renderMode8: result={}",
