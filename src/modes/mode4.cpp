...
void renderMode4(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer,
    ...
    // Project 9D ball positions into 3D space with wavefield effect
    std::vector<float> vertexData;
    vertexData.reserve(balls.size() * 6); // Position (x, y, z) + Normal (x, y, z) per ball
    float waveAmplitude = cache.empty() ? 1.0f : cache[0].value * 0.5f;
    for (const auto& ball : balls) {
        // Wavefield: modulate z based on wavePhase and cache value
        float x = ball.x * (1.0f + 0.2f * cos(wavePhase * 3.0f));
        float y = ball.y * (1.0f + 0.2f * cos(wavePhase * 3.0f));
        float z = ball.z + waveAmplitude * sin(wavePhase * 2.0f + ball.x + ball.y);
    ...
    // Begin render pass
    VkClearValue clearColor = {{{0.05f, 0.05f, 0.05f, 1.0f}}}; // Dark gray background for wavefield effect
    ...
    float musicZoom = zoomLevel * (1.0f + 0.15f * cos(wavePhase * 2.5f + randomShift));
    ...
    glm::mat4 view = glm::lookAt(
        glm::vec3(
            sin(wavePhase * 0.7f + randomShift) * 4.0f,
            cos(wavePhase * 0.7f + randomShift) * 4.0f,
            -8.0f
        ),
        glm::vec3(0.0f, 0.0f, 0.0f),
        glm::vec3(0.0f, 1.0f, 0.0f)
    );
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.4f + randomShift, glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    // Additional elements (shifted wavefield layer for depth)
    model = glm::translate(glm::mat4(1.0f), glm::vec3(sin(wavePhase * 0.3f) * 0.5f, cos(wavePhase * 0.3f) * 0.5f, 0.0f));
    pushConstants.mvp = proj * view * model;
    pushConstants.baseColor = glm::vec3(
