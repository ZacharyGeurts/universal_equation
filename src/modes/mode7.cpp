...
void renderMode7(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, [[maybe_unused]] VkBuffer vertexBuffer, [[maybe_unused]] VkCommandBuffer commandBuffer,
    ...
    if (balls.empty()) {
        amouranth->getLogger().log(Logging::LogLevel::Error, "No ball data for renderMode7",
    ...
    // Project 9D ball positions onto 3D vortex tunnel
    ...
    for (const auto& ball : balls) {
        ...
        float theta = ball.position.x * 5.0f * 3.14159f + randomShift * 0.3f;
        float radius = 2.0f / (1.0f + ball.position.z * 2.0f) + sin(wavePhase * 3.0f + randomShift) * 0.2f;
        float z = ball.position.z * 3.0f + cos(wavePhase * 2.0f + randomShift) * 0.5f;
        ...
        vertexData.push_back(cos(theta)); // Normal x
    ...
    glm::vec3 cameraPos = glm::vec3(
        0.0f,
        0.0f,
        5.0f + sin(wavePhase * 0.7f + randomShift) * 0.5f
    );
    glm::mat4 view = glm::lookAt(cameraPos, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), wavePhase * 0.6f + randomShift, glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    // Draw indexed (point-based vortex)
    uint32_t indexCount = static_cast<uint32_t>(indices.size());
    vkCmdDrawIndexed(commandBuffer, indexCount, 1, 0, 0, 0);

    // Kaleidoscopic effect (mirrored vortex)
    ...
    if (VkResult result = vkEndCommandBuffer(commandBuffer); result != VK_SUCCESS) {
        amouranth->getLogger().log(Logging::LogLevel::Error, "Failed to record command buffer for renderMode7: result={}",
