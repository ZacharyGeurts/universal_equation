...
void renderMode7(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer,
    ...
    const glm::vec4 kDefaultColor = {1.0f, 0.5f, 0.0f, 1.0f}; // Orange as default
    ...
    float scale = 1.0f + 0.2f * std::cos(wavePhase) * zoomLevel;
    model = glm::scale(model, glm::vec3(scale));
    model = glm::translate(model, glm::vec3(0.0f, std::sin(wavePhase) * 0.3f, 0.0f));
    ...
    auto renderInstance = [&](const UniversalEquation::DimensionInteraction& interaction, float strengthMod, const glm::vec4& iColor) {
        ...
        iModel = glm::translate(iModel, glm::vec3(static_cast<float>(interaction.vertexIndex) * 0.5f, std::sin(wavePhase + static_cast<float>(interaction.strength)) * 0.3f, 0.0f));
    ...
    const glm::vec4 colors[] = {
        {1.0f, 0.5f, 0.0f, 1.0f}, {0.8f, 0.4f, 0.2f, 1.0f}, {0.6f, 0.3f, 0.4f, 1.0f},
        {0.4f, 0.2f, 0.6f, 1.0f}, {0.2f, 0.1f, 0.8f, 1.0f}, {0.0f, 0.0f, 1.0f, 1.0f},
