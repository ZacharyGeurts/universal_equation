...
// Mode 3: Rendering with dark matter distortion in dimension 3
...
void renderMode3(AMOURANTH* amouranth, uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer,
    ...
    // Local function to compute oscillation value based on cache entry, with dark matter warp
    auto makeOscValue = [](const DimensionData& cacheEntry, float wavePhase) -> float {
        const float dmMod = static_cast<float>(cacheEntry.darkMatter) * 1.2f;
        const float osc = std::sin(wavePhase + dmMod) * (1.0f + dmMod * 0.5f);
    ...
    // Loop over dimensions, focus on dimension 3
    for (size_t i = 0; i < cache.size(); ++i) {
        ...
        // Define scale bias constant, increased for distortion
        constexpr float kScaleBias = 0.9f;

        const float scaleFactor = 1.0f + static_cast<float>(cache[i].darkMatter) * kScaleBias;

        // Setup transformation matrices with slight rotation for distortion effect
        glm::mat4 model = glm::rotate(glm::scale(glm::mat4(1.0f), glm::vec3(scaleFactor * zoomLevel)), wavePhase * 0.5f, glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        pc.extraData = glm::vec4(0.5f, 0.0f, 0.5f, 0.0f); // Purple tint for dark matter
