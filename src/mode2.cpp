// RenderMode2: Vulkan/GLM Hyperdrive Edition (C++17, D=2)
...
// - Dry humor: "Math’s stretching in 2D" warnings, "n-Cube’s 2D stretch"
...
static constexpr float kSphereScale = 0.28f;  // Adjusted for 2D
static constexpr float kInteractScale = 0.20f;
static constexpr float kZOffset = 5.5f;  // Slightly farther for 2D
...
namespace {
    ...
    auto buildModel(float angle, float cycleProgress, float wavePhase, float scaleFactor, const glm::vec3& posOffset = {}) {
        return [angle, cycleProgress, wavePhase, scaleFactor, posOffset](glm::mat4 base = glm::mat4(1.0f)) -> glm::mat4 {
            const float rotAngle = wavePhase * 0.35f;  // Slightly faster for 2D
            const glm::vec3 rotAxis{sinf(angle * 0.25f), cosf(angle * 0.25f), 0.5f};
    ...
    auto genOffsetPos(float dist, float strength, float angle, float cycleProgress) {
        const float offsetMult = dist * 0.85f * (1.0f + static_cast<float>(strength) * 0.25f);  // Adjusted for 2D
        return glm::vec3(
            offsetMult * cosf(angle + cycleProgress),
            offsetMult * sinf(angle + cycleProgress),
            offsetMult * 0.15f * sinf(angle * 0.6f)
        );
    }
}
...
void renderMode2(AMOURANTH* amouranth, uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer, VkBuffer indexBuffer, float zoomLevel, int width, int height, float wavePhase, const std::vector<DimensionData>& cache, VkPipelineLayout pipelineLayout) {
    ...
    if (amouranth->getSphereIndices().empty()) {
        std::cerr << "Warning: Sphere indices empty. Math’s stretching in 2D.\n";
        return;
    }
    ...
    if (cache[i].dimension != 2) {
        std::cerr << "Warning: Invalid cache for dimension 2. Math’s 2D stretch.\n";
        return;
    }
    ...
    const glm::vec3 pos{
        ...
        radius * sinf(wavePhase + static_cast<float>(i) * 0.25f) * 0.15f
    };
