...
void renderMode2(AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer,
    ...
    const glm::vec4 kDefaultColor = {0.0f, 1.0f, 0.0f, 1.0f}; // Green as default
    ...
    model = glm::rotate(model, wavePhase, glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    auto renderInstance = [&](const UniversalEquation::DimensionInteraction& interaction, float strengthMod, const glm::vec4& iColor) {
        ...
        iModel = glm::rotate(iModel, wavePhase * static_cast<float>(interaction.strength), glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    const glm::vec4 colors[] = {
        {0.0f, 1.0f, 0.0f, 1.0f}, {0.0f, 0.8f, 0.2f, 1.0f}, {0.0f, 0.6f, 0.4f, 1.0f},
        {0.0f, 0.4f, 0.6f, 1.0f}, {0.0f, 0.2f, 0.8f, 1.0f}, {0.0f, 0.0f, 1.0f, 1.0f},
        {0.2f, 0.8f, 0.2f, 1.0f}, {0.4f, 0.6f, 0.4f, 1.0f}
    };
