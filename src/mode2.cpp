...
// Mode 2: Rendering with potential energy emphasis in dimension 2
...
void renderMode2(AMOURANTH* amouranth, uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer,
    ...
    // Local function to compute oscillation value based on cache entry, emphasizing potential
    auto makeOscValue = [](const DimensionData& cacheEntry, float wavePhase) -> float {
        const float potMod = static_cast<float>(cacheEntry.potential) * 0.8f;
        const float osc = std::sin(wavePhase + potMod);
    ...
    // Loop over dimensions, focus on dimension 2
    for (size_t i = 0; i < cache.size(); ++i) {
        ...
        // Define scale bias constant, adjusted for potential emphasis
        constexpr float kScaleBias = 0.7f;

        const float scaleFactor = 1.0f + static_cast<float>(cache[i].potential) * kScaleBias;
        ...
        pc.extraData = glm::vec4(0.0f, 1.0f, 0.0f, 0.0f); // Green tint for potential
