// RenderMode6: Vulkan/GLM Hyperdrive Edition (C++17, D=6)
...
// - Dry humor: "Math’s folding in 6D" warnings, "n-Cube’s 6D fold"
...
static constexpr float kSphereScale = 0.11f;  // Smaller for 6D
static constexpr float kInteractScale = 0.08f;
static constexpr float kZOffset = 3.5f;  // Farther for 6D
...
namespace {
    ...
    auto buildModel(float angle, float cycleProgress, float wavePhase, float scaleFactor, const glm::vec3& posOffset = {}) {
        return [angle, cycleProgress, wavePhase, scaleFactor, posOffset](glm::mat4 base = glm::mat4(1.0f)) -> glm::mat4 {
            const float rotAngle = wavePhase * 0.55f;  // Faster for 6D
            const glm::vec3 rotAxis{sinf(angle * 0.45f), cosf(angle * 0.45f), 0.1f};
    ...
    auto genOffsetPos(float dist, float strength, float angle, float cycleProgress) {
        const float offsetMult = dist * 0.65f * (1.0f + static_cast<float>(strength) * 0.45f);  // Adjusted for 6D
        return glm::vec3(
            offsetMult * cosf(angle + cycleProgress),
            offsetMult * sinf(angle + cycleProgress),
            offsetMult * 0.35f * sinf(angle * 1.0f)
        );
    }
}
...
void renderMode6(AMOURANTH* amouranth, uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer, VkBuffer indexBuffer, float zoomLevel, int width, int height, float wavePhase, const std::vector<DimensionData>& cache, VkPipelineLayout pipelineLayout) {
    ...
    if (amouranth->getSphereIndices().empty()) {
        std::cerr << "Warning: Sphere indices empty. Math’s folding in 6D.\n";
        return;
    }
    ...
    if (cache[i].dimension != 6) {
        std::cerr << "Warning: Invalid cache for dimension 6. Math’s 6D fold.\n";
        return;
    }
    ...
    // Angle/scale/radius
    const float dim = 6.0f;
    const float angle = wavePhase + dim * 2.0f * glm::pi<float>() / kMaxRenderedDimensions;
    ...
    const glm::vec3 pos{
        ...
        radius * sinf(wavePhase + static_cast<float>(i) * 0.45f) * 0.35f
    };
