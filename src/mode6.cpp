...
void renderMode6(AMOURANTH* amouranth, uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer, VkBuffer indexBuffer, float zoomLevel, int width, int height, float wavePhase, const std::vector<DimensionData>& cache, VkPipelineLayout pipelineLayout) {
    ...
    for (size_t i = 0; i < kMaxRenderedDimensions; ++i) {
        ...
        if (cache[i].dimension != 6) continue; // Only render dimension 6

        // Toroidal oscillation
        float osc = 1.0f + 0.3f * sinf(wavePhase * (1.0f + static_cast<float>(cache[i].darkEnergy) * 0.7f));
        float value = static_cast<float>(cache[i].observable * osc);
        value = glm::clamp(value, 0.01f, 2.0f);

        // Toroidal positioning
        float majorAngle = wavePhase + (i + 1) * 2.0f * glm::pi<float>() / kMaxRenderedDimensions;
        float minorAngle = wavePhase * (1.0f + static_cast<float>(cache[i].darkMatter) * 0.5f);
        float majorRadius = 4.0f;
        float minorRadius = 1.0f * (1.0f + static_cast<float>(cache[i].darkMatter) * 0.3f);
        glm::vec3 pos = glm::vec3(
            (majorRadius + minorRadius * cosf(minorAngle)) * cosf(majorAngle + cycleProgress),
            (majorRadius + minorRadius * cosf(minorAngle)) * sinf(majorAngle + cycleProgress),
        ...
        model = glm::rotate(model, wavePhase * 0.6f, glm::vec3(1.0f, 0.0f, 0.0f));

        // Color with toroidal effect
        glm::vec3 baseColor = glm::vec3(
            0.5f + 0.5f * cosf(wavePhase + i * 0.9f + cycleProgress),
            0.6f + 0.4f * sinf(wavePhase + i * 0.7f),
    ...
    // Interactions for dimension 6
    amouranth->setCurrentDimension(6);
    auto pairs = amouranth->getInteractions();
    if (pairs.empty()) {
        std::cerr << "Warning: No interactions for dimension 6\n";
        ...
        for (const auto& pair : pairs) {
            if (amouranth->getMode() != 6) continue;
