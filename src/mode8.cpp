// RenderMode8: Vulkan/GLM Hyperdrive Edition (C++17, D=8)
...
// - Dry humor: "Math’s imploding in 8D" warnings, "n-Cube’s 8D implode"
...
static constexpr float kSphereScale = 0.05f;  // Smaller for 8D
static constexpr float kInteractScale = 0.04f;
static constexpr float kZOffset = 2.5f;  // Farther for 8D
...
namespace {
    ...
    auto buildModel(float angle, float cycleProgress, float wavePhase, float scaleFactor, const glm::vec3& posOffset = {}) {
        return [angle, cycleProgress, wavePhase, scaleFactor, posOffset](glm::mat4 base = glm::mat4(1.0f)) -> glm::mat4 {
            const float rotAngle = wavePhase * 0.65f;  // Faster for 8D
            const glm::vec3 rotAxis{sinf(angle * 0.55f), cosf(angle * 0.55f), -0.1f};
    ...
    auto genOffsetPos(float dist, float strength, float angle, float cycleProgress) {
        const float offsetMult = dist * 0.55f * (1.0f + static_cast<float>(strength) * 0.55f);  // Adjusted for 8D
        return glm::vec3(
            offsetMult * cosf(angle + cycleProgress),
            offsetMult * sinf(angle + cycleProgress),
            offsetMult * 0.45f * sinf(angle * 1.2f)
        );
    }
}
...
void renderMode8(AMOURANTH* amouranth, uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer, VkBuffer indexBuffer, float zoomLevel, int width, int height, float wavePhase, const std::vector<DimensionData>& cache, VkPipelineLayout pipelineLayout) {
    ...
    if (amouranth->getSphereIndices().empty()) {
        std::cerr << "Warning: Sphere indices empty. Math’s imploding in 8D.\n";
        return;
    }
    ...
    if (cache[i].dimension != 8) {
        std::cerr << "Warning: Invalid cache for dimension 8. Math’s 8D implode.\n";
        return;
    }
    ...
    const glm::vec3 pos{
        ...
        radius * sinf(wavePhase + static_cast<float>(i) * 0.55f) * 0.45f
    };
