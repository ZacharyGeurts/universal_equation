// Fancy-Ass RenderMode8: Vulkan/GLM Hyperdrive Edition (C++17, D=8)
// - Renders 8D n-Cube as spheres, modulated by observable/darkEnergy
// - Constexpr constants, inlined sin/cos, GLM swizzles for cache-friendly math
// - Dry humor: "Math’s having a cosmic breakdown" warnings, "n-Cube’s 8D despair"
...
static constexpr float kSphereScale = 0.20f;  // Smaller for 8D
static constexpr float kInteractScale = 0.10f;
static constexpr float kZOffset = 20.0f;  // Further for 8D
...
namespace {
    ...
    auto buildModel(float angle, float cycleProgress, float wavePhase, float scaleFactor, const glm::vec3& posOffset = {}) {
        return [angle, cycleProgress, wavePhase, scaleFactor, posOffset](glm::mat4 base = glm::mat4(1.0f)) -> glm::mat4 {
            const float rotAngle = wavePhase * 1.0f;  // Faster for 8D
            const glm::vec3 rotAxis{sinf(angle * 0.9f), cosf(angle * 0.9f), 0.05f};
    ...
    auto genBaseColor(float wavePhase, size_t i, float cycleProgress) {
        return glm::vec3(
            0.9f + 0.1f * cosf(wavePhase + static_cast<float>(i) * 1.6f + cycleProgress),
            0.8f + 0.05f * sinf(wavePhase + static_cast<float>(i) * 1.3f),
            1.0f - 0.05f * cosf(wavePhase * 1.1f + static_cast<float>(i))
        );
    }
...
void renderMode8(AMOURANTH* amouranth, uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer, VkBuffer indexBuffer, float zoomLevel, int width, int height, float wavePhase, const std::vector<DimensionData>& cache, VkPipelineLayout pipelineLayout) {
    ...
    if (amouranth->getSphereIndices().empty()) {
        std::cerr << "Warning: Sphere indices empty. Math’s having a cosmic breakdown.\n";
        return;
    }
    ...
    if (cache[i].dimension != 8) {
        std::cerr << "Warning: Invalid cache for dimension 8. Math’s 8D despair.\n";
        return;
    }
    ...
    // Angle/scale/radius
    const float angle = wavePhase + 8 * 2.0f * glm::pi<float>() / kMaxRenderedDimensions;
    ...
    const glm::vec3 pos{
        ...
        radius * sinf(wavePhase + static_cast<float>(i) * 0.9f) * 0.45f
    };
    ...
    const PushConstants pushConstants = {
        model, view, proj, baseColor, value, 8.0f, wavePhase, cycleProgress,
    ...
    if (pairs.empty()) {
        ...
        const PushConstants fallbackPush = {fallbackModel, view, proj, kDefaultColor, 0.4f, 8.0f, wavePhase, cycleProgress, 0.4f, 0.4f};
        vkCmdPushConstants(commandBuffer, pipelineLayout,
                           VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                           0, sizeof(PushConstants), &fallbackPush);
        vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(amouranth->getSphereIndices().size()), 1, 0, 0, 0);
        std::cerr << "Warning: No interactions for dimension 8. Math’s lonely.\n";
        return;
    }

    // Mode guard
    const auto modeGuard = [&amouranth]() { return amouranth->getMode() == 8; };
    ...
    std::for_each(filteredPairs.begin(), filteredPairs.end(), [&](const UniversalEquation::DimensionInteraction& pair) {
        ...
        const float iAngle = wavePhase + static_cast<float>(pair.vertexIndex) * 2.2f + static_cast<float>(pair.distance) * 0.9f;
        ...
        const glm::vec3 iColor{
            ...
            0.9f - 0.15f * cosf(iAngle * 1.9f),
            0.95f - 0.1f * sinf(iAngle * 1.5f)
        };
        ...
        const PushConstants iPush = {iModel, view, proj, iColor, strengthMod, 8.0f, wavePhase, cycleProgress,
