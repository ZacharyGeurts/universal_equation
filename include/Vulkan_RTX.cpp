...
// Definitions of VulkanRTX static function pointers.
...
// Implementation of VulkanRTX methods.
void VulkanRTX::initializeRTX(
    ...
    // Load required ray tracing extension functions.
...
void VulkanRTX::createRayTracingPipeline(VkDevice device, VkPipeline& pipeline, VkPipelineLayout& rtPipelineLayout) {
    ...
    VkShaderModule raygenShader = createShaderModule(device, "assets/shaders/raygen.spv");
    VkShaderModule missShader = createShaderModule(device, "assets/shaders/miss.spv");
    VkShaderModule closestHitShader = createShaderModule(device, "assets/shaders/closest_hit.spv");
    ...
    try { anyHitShader = createShaderModule(device, "assets/shaders/any_hit.spv"); } catch (...) {}
    try { intersectionShader = createShaderModule(device, "assets/shaders/intersection.spv"); } catch (...) {}
    try { callableShader = createShaderModule(device, "assets/shaders/callable.spv"); hasCallable = true; } catch (...) {}
...
void VulkanRTX::createShaderBindingTable(VkDevice device, VkPhysicalDevice physicalDevice, VkPipeline pipeline, ShaderBindingTable& sbt) {
    ...
    try {
        VkShaderModule mod = createShaderModule(device, "assets/shaders/intersection.spv");
        vkDestroyShaderModule(device, mod, nullptr);
        hasIntersection = true;
    } catch (...) {}
    try {
        VkShaderModule mod = createShaderModule(device, "assets/shaders/callable.spv");
        vkDestroyShaderModule(device, mod, nullptr);
        hasCallable = true;
    } catch (...) {}
...
void VulkanRTX::createBottomLevelAS(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool commandPool, VkQueue queue,
                                    VkBuffer vertexBuffer, VkBuffer indexBuffer, uint32_t vertexCount, uint32_t indexCount,
...
void VulkanRTX::createTopLevelAS(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool commandPool, VkQueue queue,
...
void VulkanRTX::createBuffer(VkDevice device, VkPhysicalDevice physicalDevice, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags props, VkBuffer& buffer, VkDeviceMemory& memory) {
...
VkShaderModule VulkanRTX::createShaderModule(VkDevice device, const std::string& filename) {
