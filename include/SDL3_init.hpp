...
public:
    ...
    SDL3Initializer() : logFile("sdl3_initializer.log") {
    ...
    void initialize(const char* title, int w, int h, Uint32 flags = SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE,
                    ...
                    bool enableValidation = true, bool preferNvidia = true) {
...
private:
    ...
    void initSDL(const char* title, int w, int h, Uint32 flags, bool rt, const std::string& fontPath, bool enableValidation, bool preferNvidia) {
        ...
        std::vector<std::string> videoDrivers = {"x11", "wayland", ""}; // Try X11, Wayland, then default
        if (platform == "Linux") {
            // Configure NVIDIA PRIME render offload for hybrid graphics
            std::string icdPath = "/usr/share/vulkan/icd.d/nvidia_icd.json:/usr/share/vulkan/icd.d/radeon_icd.json";
            setenv("VK_ICD_FILENAMES", icdPath.c_str(), 1);
            setenv("__NV_PRIME_RENDER_OFFLOAD", "1", 1);
            setenv("__GLX_VENDOR_LIBRARY_NAME", "nvidia", 1);
            setenv("__VK_LAYER_NV_optimus", "NVIDIA_only", 1);
            logMessage("Configured environment for NVIDIA PRIME render offload to select high-performance GPU");
            logMessage("VK_ICD_FILENAMES set to: " + icdPath);
            // Validate ICD files
            for (const auto& icd : {"nvidia_icd.json", "radeon_icd.json"}) {
                std::filesystem::path icdFile = "/usr/share/vulkan/icd.d/" + std::string(icd);
                if (std::filesystem::exists(icdFile)) {
                    logMessage("ICD file found: " + icdFile.string());
                } else {
                    logMessage("Warning: ICD file not found: " + icdFile.string());
                }
            }
            ...
            logMessage("Selecting audio driver for Linux: " + std::string(audioDriver));
            SDL_SetHint(SDL_HINT_AUDIO_DRIVER, audioDriver);
        } else {
            logMessage("Non-Linux platform detected; using default SDL drivers");
            videoDrivers = {""};
        }
        ...
        if (!hasSurfaceExtension) {
            logMessage("Warning: VK_KHR_surface extension not supported; proceeding to device selection");
        }
        if (!hasPlatformSurfaceExtension) {
            logMessage("Warning: No platform-specific surface extensions supported; proceeding to device selection");
        }

        // Try each video driver until a valid window and Vulkan surface (if applicable) are created
        VkSurfaceKHR surface = VK_NULL_HANDLE;
        bool surfaceCreated = false;
        std::string lastError;
        bool useVulkan = (flags & SDL_WINDOW_VULKAN) != 0;
        for (const auto& driver : videoDrivers) {
            if (!driver.empty()) {
                SDL_SetHint(SDL_HINT_VIDEO_DRIVER, driver.c_str());
                logMessage("Trying video driver: " + driver);
            } else {
                logMessage("Trying default video driver");
            }
            ...
            if (!m_window) {
                lastError = "SDL_CreateWindow failed with driver " + (driver.empty() ? "default" : driver) + ": " + std::string(SDL_GetError());
            ...
            if (!exts || !extCount) {
                lastError = "SDL_Vulkan_GetInstanceExtensions failed with driver " + (driver.empty() ? "default" : driver) + ": " + std::string(SDL_GetError());
                logMessage(lastError);
                m_window.reset();
                continue;
            }
            ...
            if (platform == "Linux") {
                platformSurfaceExtension = (driver == "wayland" && std::find(platformSurfaceExtensions.begin(), platformSurfaceExtensions.end(), VK_KHR_WAYLAND_SURFACE_EXTENSION_NAME) != platformSurfaceExtensions.end()) ? 
                    VK_KHR_WAYLAND_SURFACE_EXTENSION_NAME : 
                    (std::find(platformSurfaceExtensions.begin(), platformSurfaceExtensions.end(), VK_KHR_XLIB_SURFACE_EXTENSION_NAME) != platformSurfaceExtensions.end()) ? 
                    VK_KHR_XLIB_SURFACE_EXTENSION_NAME : "";
            }

            if (!platformSurfaceExtension.empty()) {
                logMessage("Adding Vulkan surface extension: " + platformSurfaceExtension);
                extensions.push_back(platformSurfaceExtension.c_str());
            } else {
                logMessage("Warning: No suitable platform-specific Vulkan surface extension available for driver: " + (driver.empty() ? "default" : driver));
            }
            ...
            // Conditionally include VK_KHR_surface
            ...
            if (result != VK_SUCCESS) {
                lastError = "vkCreateInstance failed with driver " + (driver.empty() ? "default" : driver) + ": " + std::to_string(result);
                logMessage(lastError);
                m_window.reset();
                continue;
            }
            ...
            logMessage("Creating Vulkan surface with driver: " + (driver.empty() ? "default" : driver));
            if (!SDL_Vulkan_CreateSurface(m_window.get(), m_instance.get(), nullptr, &surface)) {
                lastError = "SDL_Vulkan_CreateSurface failed with driver " + (driver.empty() ? "default" : driver) + ": " + std::string(SDL_GetError());
                logMessage(lastError);
                m_instance.reset();
                m_window.reset();
                continue;
            ...
            logMessage("Vulkan surface created successfully with driver: " + (driver.empty() ? "default" : driver));
            surfaceCreated = true;
            break;
        }

        if (!surfaceCreated && useVulkan) {
            logMessage("Failed to create Vulkan surface with any driver: " + lastError);
            logMessage("Falling back to non-Vulkan rendering");
            flags &= ~SDL_WINDOW_VULKAN; // Remove Vulkan flag
            for (const auto& driver : videoDrivers) {
                if (!driver.empty()) {
                    SDL_SetHint(SDL_HINT_VIDEO_DRIVER, driver.c_str());
                    logMessage("Trying video driver for non-Vulkan rendering: " + driver);
                } else {
                    logMessage("Trying default video driver for non-Vulkan rendering");
                }

                logMessage("Creating SDL window with flags (non-Vulkan): " + std::to_string(flags));
                m_window = std::unique_ptr<SDL_Window, SDLWindowDeleter>(SDL_CreateWindow(title, w, h, flags));
                if (m_window) {
                    logMessage("Non-Vulkan window created successfully with driver: " + (driver.empty() ? "default" : driver));
                    surfaceCreated = true;
                    break;
                }
                lastError = "SDL_CreateWindow failed with driver " + (driver.empty() ? "default" : driver) + ": " + std::string(SDL_GetError());
        ...
        if (!surfaceCreated) {
            logMessage("Failed to create window with any driver: " + lastError);
            throw std::runtime_error(lastError);
        }

        // Select physical device if using Vulkan
        if (useVulkan) {
            ...
            try {
                ...
                logMessage("Falling back to non-Vulkan rendering");
                m_surface.reset(); // Destroy surface first
                m_instance.reset(); // Then destroy instance
                flags &= ~SDL_WINDOW_VULKAN;
                m_window.reset();
                for (const auto& driver : videoDrivers) {
                    if (!driver.empty()) {
                        SDL_SetHint(SDL_HINT_VIDEO_DRIVER, driver.c_str());
                        logMessage("Retrying video driver for non-Vulkan rendering: " + driver);
                    } else {
                        logMessage("Retrying default video driver for non-Vulkan rendering");
                    }
                ...
                if (!m_window) {
                    logMessage("Failed to create non-Vulkan window: " + lastError);
                    throw std::runtime_error(lastError);
                }
            }
        }
