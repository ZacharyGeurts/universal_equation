...
public:
    ...
    static void createRayTracingPipeline(VkDevice device, VkPipeline& pipeline, VkPipelineLayout& rtPipelineLayout) {
        VkShaderModule raygenShader = createShaderModule(device, "bin/raygen.rgen.spv");
        VkShaderModule missShader = createShaderModule(device, "bin/miss.rmiss.spv");
        VkShaderModule closestHitShader = createShaderModule(device, "bin/closesthit.rchit.spv");
    ...
    static void createTopLevelAS(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool commandPool, VkQueue queue,
                                 ...
                                 VkAccelerationStructureKHR& as, VkBuffer& asBuffer, VkDeviceMemory& asMemory) {
        if (geometries.empty() || buildRanges.empty()) {
            throw std::runtime_error("Geometries or build ranges are empty for TLAS");
        }
...
private:
    ...
    static void createLogicalDevice(VkPhysicalDevice physicalDevice, VkDevice& device, VkQueue& graphicsQueue, VkQueue& presentQueue, uint32_t graphicsFamily, uint32_t presentFamily) {
        ...
        for (const auto& ext : exts) {
            ...
            if (strcmp(ext.extensionName, VK_KHR_RAY_TRACING_PIPELINE_EXTENSION_NAME) == 0) hasRayTracing = true;
            if (strcmp(ext.extensionName, VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME) == 0) hasAccelerationStructure = true;
        ...
        if (!hasAccelerationStructure) throw std::runtime_error("No acceleration structure support");
        if (!hasDeferredHostOps) throw std::runtime_error("No deferred host operations support");
        if (!hasBufferDeviceAddress) throw std::runtime_error("No buffer device address support");
    ...
    static void createSwapchain(VkPhysicalDevice physicalDevice, VkDevice device, VkSurfaceKHR surface, VkSwapchainKHR& swapchain,
        ...
        VkExtent2D extent = caps.currentExtent.width != UINT32_MAX ? caps.currentExtent : VkExtent2D{
            std::min(std::max(static_cast<uint32_t>(width), caps.minImageExtent.width), caps.maxImageExtent.width),
            std::min(std::max(static_cast<uint32_t>(height), caps.minImageExtent.height), caps.maxImageExtent.height)
        };
    ...
    static VkShaderModule createShaderModule(VkDevice device, const std::string& filename) {
        ...
        file.close();
        size_t paddedSize = (buffer.size() + 3) & ~3;
    ...
    static void createGraphicsPipeline(VkDevice device, VkRenderPass renderPass, VkPipeline& pipeline, VkPipelineLayout& pipelineLayout, int width, int height) {
        VkShaderModule vert = createShaderModule(device, "bin/vertex.spv");
        VkShaderModule frag = createShaderModule(device, "bin/fragment.spv");
        ...
        std::cerr << "Graphics PushConstants size: " << sizeof(PushConstants) << " bytes\n";
    ...
    static void createVertexBuffer(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool commandPool, VkQueue graphicsQueue,
        ...
        createBuffer(device, physicalDevice, size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertexBuffer, vertexBufferMemory);
    ...
    static void createIndexBuffer(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool commandPool, VkQueue graphicsQueue,
        ...
        vkUnmapMemory(device, stagingMem);
        createBuffer(device, physicalDevice, size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBuffer, indexBufferMemory);
